...
## 14. Изменения при реализации

Утверждённые разделы выше не переписываются. Здесь — всё, чем код (ветка
`feat/hierarchical-memory`, задачи 1–12) отличается от §4–§9, с причиной. Где
спека и код расходятся, верен код; пользовательское описание — в
`docs/HORAE.md` и `docs/API.md`.

### К §4.1 (константы и данные)

- `MemoryConfig.batch_max_chars` не ниже `_MIN_BATCH_CHARS = 2000`; `None`, `0`
  и мусор — умолчание 80 000 (`__post_init__`): шапка строки и маркер середины
  сами занимают около сотни символов, а `MEMORY_BATCH_CHARS` снизу не ограничен.
- Новое поле `MemoryConfig.output_tokens: int | None = None` — лимит вывода
  модели памяти (её `max_tokens`); по нему `merge_block` сверяет снимок до
  слияния (см. §4.5). `None` — проверки нет.
- `ScanResult.status` — ещё и `"conflict"` (см. §4.8); `MemoryLLMError` несёт
  `retry_after`.
- Новое исключение `MemoryCancelled`: отмена прогона посреди паузы (см. §4.4).
  Это не `asyncio.CancelledError` — ту ядро не перехватывает никогда.
- `MAX_SNAPSHOT_CHARS = 200 000` — нижняя граница: порог «неправдоподобно
  длинного» снимка — `max(200 000, 8 × snapshot_tokens)`. Русский текст —
  около 1,9 символа на токен, и при бюджете больше ~100 тыс. токенов снимок в
  пределах бюджета отвергался бы.

### К §4.2 (нормализация)

- HTML-теги снимаются только известные (белый список имён и атрибутов по
  синтаксису HTML): ремарки `<OOC: …>`, `<time skip>`, «x <y and z> w» остаются
  текстом. Голые `<text>`, `<stop>`, `<line>`, `<main>`, `<nav>`, `<time>`,
  `<summary>` снимаются как теги — осознанно оставлено.
- Парные блоки (`<think>`, `<thinking>`, `<style>`, `<script>`, `<!--…-->`) режет
  линейный сканер `_drop_paired`, а не регулярка `<x>[\s\S]*?</x>`: на тысячах
  незакрытых открытий регулярка работала квадратично (80 000 символов — 3–5 с
  в цикле событий). Атрибуты открывающего тега — не длиннее 512 символов.
- Сообщение, от которого после чистки ничего не осталось (одни `<think>`), идёт в
  пакет без строки: модель его не видит, но указатель его проходит.
- «Ответ ассистента, разбирающего файл, идёт в тот же пакет» не гарантировано:
  сообщение с файлом может быть последним в пакете или перед окном.
- Сигнатура — `normalize_message(msg, max_chars)`.

### К §4.3 (пакеты)

- `plan_batch(messages, batch_size, max_chars)`, `plan_batches(…)` — параметры
  явные; менеджер зовёт их со своим `MemoryConfig`.
- Пакет из одних пустых сообщений `merge_block` возвращает без вызова модели:
  вливать нечего, а указатель должен их пройти.

### К §4.4 (вызов модели)

- `call` сериализован `asyncio.Lock`: параллельные вызовы одного менеджера иначе
  прочли бы один `_last_end` и ушли к провайдеру разом. Конец вызова отмечается
  при любом исходе, кроме отмены (в том числе когда колбэк сам бросил
  `MemoryLLMError`).
- Повтор ждёт не меньше паузы `delay_ms`; `Retry-After` ограничен 300 с.
- `MemoryLLMError` от колбэка возвращается `classify_error` как есть
  (сервис классифицирует обрыв и отказ из-за `max_tokens` сам).
- Классификация дополнена именами классов litellm (`APITimeoutError`,
  `UnprocessableEntityError` и др.; проверено на настоящих исключениях).
- Перед каждым запросом, если UI видит другое, уходит событие с фазой запроса
  (`merge`/`compact`): иначе опрос весь платный запрос показывал «пауза» или
  замерший «повтор через 2 с».
- **Отмена прерывает паузы.** Прогон передаёт `cancel` менеджеру; пауза между
  запросами и пауза перед повтором ждут `min(сон, cancel)`, после отмены новых
  запросов нет — `MemoryCancelled`. Уже начатый запрос доводится (он оплачен).
  В спеке отмена проверялась только между пакетами, и «Остановить»/«Сбросить»
  ждали до 300 с × 4 повтора.

### К §4.5 (слияние)

- **Обрыв лимитом вывода — `length`, а не брак.** Открытая обёртка без закрытия
  (`extract_snapshot` → `truncated`) даёт `MemoryLLMError("length",
  retryable=False)` без корректирующих ходов: ход с тем же лимитом оборвался бы
  снова. Непустой ответ, оборванный `max_tokens`, сервис распознаёт по
  `finish_reason` (см. §6.1). В спеке путь «сжать и повторить» срабатывал только
  на пустой ответ, и обычный обрыв давал три полноразмерных вызова и отказ — на
  каждом ходу.
- Путь `length`: `compact(state, min_fold=1, fold_arcs=True)` — со сворачиванием
  и старых арок (`COMPACT_ARCS_PROMPT`), затем один повтор; повторный обрыв —
  ошибка наверх. Отмена между сжатием и повтором — `MemoryCancelled`.
- **Сверка с лимитом вывода до слияния** (`_fit_output`, при известном
  `output_tokens` и снимке новой схемы): разделы 2–4 (`_guarded_tokens`) сами не
  меньше лимита — `MemoryLLMError("length", «реестр и списки больше лимита
  вывода модели памяти …»)` без вызова модели; весь снимок не меньше лимита —
  сначала сжатие хроники, не помогло — та же ошибка.
- `extract_snapshot` берёт последний ЗАКРЫТЫЙ кандидат обёртки со снимком по
  схеме (модель упоминает тег и до снимка, и после); иначе последний закрытый.
- В корректирующий ход идёт только последний брак (исходный запрос + сырой
  ответ + «Ответ отклонён: …»), а не все попытки.
- Предупреждение «снимок превышает бюджет» убирается из итога прогона, как
  только снимок снова в бюджете; новое такое предупреждение заменяет прежнее.

### К §4.6 (страж)

- Сопоставление — **мультимножество**: одна запись нового снимка покрывает одну
  старую; сначала точные совпадения ключа, потом похожие (Жаккар ≥ 0,6, из
  свободных — самая похожая). Для ключей, повторяющихся в старом разделе, —
  расширенный ключ: вся первая строка без маркера и хвостовых «(#id)»/«(было:
  …)» (снимаются линейным сканером `_strip_trailing_notes`). В спеке записи с
  одним ключом прикрывали друг друга («Linkin Park — «Numb»» / «… «In the End»»,
  «Кольцо (золотое)» / «(серебряное)»), и потеря была тихой.
- Одинаковые старые записи считаются одной; лишний возврат (модель слила две
  записи в одну строку) безопаснее потери.

### К §4.7 (сжатие)

- Из ответа сжатия берётся **только хроника**:
  `render_snapshot({**parse_sections(state), SEC_CHRONICLE: <хроника ответа>})`;
  разделы 2–4 — из входа символ в символ, страж после сжатия не нужен. В спеке
  ответ шёл в снимок целиком, и «ужатые» атрибуты персонажа принимались молча.
- Нижняя граница (`_chronicle_loss`): пустая хроника или хроника без последнего
  номера сообщения самой свежей записи — отказ с предупреждением, снимок прежний.
- Цель в промпте — 80 % бюджета (`_COMPACT_TARGET`), модель зовётся, только
  когда для свёртки накопилось `max(2, keep // 2)` записей старше последних
  `keep` (`_fold_threshold`, арки не в счёт). Параметры `min_fold`, `fold_arcs`.
- Отмена посреди паузы перед сжатием — прежний снимок без предупреждения.

### К §4.8 (цикл)

- Сигнатура: `scan_and_compress_history(source, state="", *, batch_size=None,
  delay_ms=None, on_progress=None, cancel=None, max_batches=None,
  retry_conflicts=True)`. Буквальный вызов ТЗ
  `scan_and_compress_history(chat_history, batch_size=20, delay_ms=1500)` работает:
  `batch_size`/`delay_ms` — переопределения `MemoryConfig` на один прогон.
- `max_batches` считает попытки, дошедшие до `commit` (принятые и отвергнутые).
- `retry_conflicts=False` (ежеходный проход): отвергнутый `commit` завершает
  прогон статусом `"conflict"` — следующий ход начнёт кусок заново; `True`
  (задание) — до трёх отказов подряд, затем `SourceConflictError`.
- `MemoryCancelled` из `merge_block` — статус `"cancelled"`, пакет не пишется.

### К §4.10 и §6.1 (оценка токенов, колбэк LLM)

- Сервис передаёт ядру `horae_memory.count_tokens` — тот же tiktoken, но **без
  кэша**: каждый снимок уникален, а `lru_cache` держал бы их тексты (сотни МБ
  после большой пересборки). `estimate_tokens` кэширует в два яруса: до 20 000
  символов — 4096 записей, длиннее — 16.
- `out_tokens` прижат к лимиту вывода модели памяти по карте LiteLLM
  (`llm_gateway.model_max_output_tokens`, имя пробуется и без `litellm_proxy/`), а
  рабочий `snapshot_tokens` — к ⌊(лимит − 1024) / 1,4⌋. Иначе у модели по
  умолчанию `gpt-4o` (16 384) каждый запрос с 17 824 получал 400. Модель
  неизвестна, а провайдер отверг `max_tokens` (400 со словом
  `max_tokens`/`maxOutputTokens`) — один повтор с `DEFAULT_MAX_TOKENS` и
  предупреждение, дальше в прогоне — сразу с ним.
- Колбэк читает `finish_reason` стрима через `llm_gateway.finish_reason_probe()`
  (`ContextVar`; сигнатуры `complete`/`stream_completion` не меняются) и при
  `length`/`max_tokens` бросает `MemoryLLMError("length")` — кроме запроса фактов:
  обрывок списка фактов полезен.

### К §5 (промпты)

- `MASTER_STATE_PROMPT` получил правило 10: каждая запись — строка у края с «- »,
  списки — под `### Название` (на этом формате держится страж).
- `COMPACT_PROMPT` просит сохранить имена, числа, решения объединяемых записей и
  покрыть диапазонами все записи; цель `{budget}` — 80 % бюджета.
- Новые `COMPACT_ARCS_PROMPT` (путь `length`) и `CORRECTION_PROMPT`.

### К §6.2 (хранение)

- `adopt_summary(entry, content, last_id, *, tokens=None, updated_by, warnings)`
  (публичное имя; в main — псевдоним `_adopt_summary`). Запись с пустым снимком
  не включается (`enabled = bool(content.strip())`). Убирает и
  `meta.last_error`.
- Новые ключи `meta`: `facts_upto` (до какого сообщения разобраны факты),
  `last_error` (`{message, kind, at, failures}` — сбой ежеходного прохода),
  `rebuild.paused` (пересборку остановили кнопкой).
- Остановленный буфер (`paused`) — не цель ежеходных проходов и «Догнать»: они
  пишут в живой снимок, буфер ждёт «Продолжить» (в спеке «Остановить» было
  паузой за чужой счёт: проходы продолжали буфер и сами подменяли им снимок).

### К §6.3 (`DbBatchSource`)

- Сигнатура: `DbBatchSource(session_id, *, mode, threshold, manager, want_facts,
  connection, window, alive=None)` — без `deps` и `estimate_tokens`; `alive` —
  задание ещё владеет чатом (id удалённого чата SQLite отдаёт новому).
- Пустота сообщения — одно SQL-условие `_has_content` для загрузки, бэклога и
  сверки куска.
- Подмена буфером ниже порога — не для `mode="rebuild"` (задание подменяет само
  в конце) и не для остановленного буфера.
- Факты: «разобраны до» = `max(meta.facts_upto, max(source_message_id))`;
  удачный разбор (даже без фактов) двигает `facts_upto`. Снимок коммитится ДО
  запроса фактов; если тот не удался (сбой, отмена, остановка сервера), пропуск
  доизвлекает следующий пакет — до двух пакетов назад (`_facts_gap`), самые
  свежие строки в пределах `batch_max_chars`. В спеке факты такого пакета
  терялись навсегда.
- Удачная запись (любым путём) убирает ошибку ежеходного прохода.

### К §6.4 (ежеходный проход)

- Сбой прохода записывается в `meta.last_error` (у чата без записи снимка — в
  памяти процесса); следующий проход ждёт `min(6 ч, 10 мин · 2^(failures−1))` от
  `at`. Задания паузу не ждут. В спеке — «лог и повтор на следующем ходу», то
  есть тот же платный провал на каждом ходу.
- Идущий проход получает событие отмены (`_turn_cancels`): его ставит сброс
  памяти.

### К §6.5 (задания)

- `MemoryJob`: `queued_at` (постановка) и `started_at` (переход в `running`),
  `snapshot_tokens` (живой снимок после задания; `None`, пока идёт или если
  упало).
- **Одно задание на процесс** (`_jobs_gate`), остальные `queued`: пауза
  действует внутри прогона, и задания разных чатов разом множили частоту
  запросов к общему ключу. Задание, ждущее очереди, чат не занимает.
- Задание без записи памяти — `done` с `processed: 0` и предупреждением:
  `EMPTY_REBUILD_WARNING` (буфер пересборки пуст — прежний снимок оставлен),
  «сжимать нечего — весь чат в окне», «догонять нечего — всё, что старше окна,
  уже в снимке».
- Отмена: паузы обрываются сразу, слитый пакет сохраняется, буфер пересборки
  помечается `paused`. «Продолжить» (`resume`) пометку снимает.
- `forget_job(session_id)` — при удалении чата: снять задание и убрать его из
  реестра; задание завершается «чат удалён — памяти больше некуда записываться».
- Тексты ошибок — одни у `job.error` и `snapshot.last_error` (`describe_error`).

### К §6.6 (статус, сброс, экспорт)

- `status(db, session_id, connection=None)`; новые поля: `job.queued_at`,
  `job.snapshot_tokens`, `snapshot.last_error`, `snapshot.retry_after`,
  `staging.paused`, `settings.max_snapshot_tokens` (`null` — модель неизвестна);
  в `snapshot.warnings` — ещё предупреждение, если бюджет больше, чем пишет
  модель памяти. У сводки до 2.4.0 `staging` есть и без явного буфера
  (`manual: false`). `snapshot.tokens` — по тексту (снимок правят руками).
- `purge` отменяет и идущий ежеходный проход (после текущего запроса к модели).
- Имя файла экспорта: название ≤ 80 символов; без названия —
  `memory-<id>.md`; заголовок — ASCII-имя плюс RFC 5987 `filename*`.

### К §7 (API)

- Нет чата → 404 раньше проверки доступа (403).
- `POST …/memory/cancel` — не «между пакетами», а сразу после текущего запроса
  к модели (паузы обрываются).
- `GET …/context`: у группового чата `tiers: null` и `tiers_unavailable:
  "group"` — ход группы собирает `group_chat.build_group_messages`, и монитор
  личного конвейера занижал его цену в разы. В `tiers` есть `pct_budget` и
  `pct_limit`.
- **Tier 2 в бюджете хода.** `assemble_context` собирает до обрезки истории всё,
  что от неё не зависит (системный промпт, аватары, база знаний с медиа, блок
  снимка, статичный хвост, текущее сообщение), и резервирует его вес в
  `stable_trim_start`. В спеке резерв — только системный промпт и реплика, и
  снимок шёл сверх бюджета (при 30 000 — до 41 тыс.). Не резервируются факты
  (≤ TOP_K × MAX_FACT_CHARS, зависят от обрезки) и строка манифеста.

### К §8 (настройки)

- Поле «Бюджет снимка» в интерфейсе — до 40 000 (сервер принимает до 200 000).
- Ключи `memory_batch`, `memory_delay_ms`, `memory_snapshot_tokens` интерфейс
  пишет в `ui`, только когда человек сам поменял поле; до тех пор действуют
  значения `.env` (их отдаёт `status.settings`).
- `tests/conftest.py` задаёт и `MEMORY_MAX_RETRIES=0` (тест со сбоем фактов иначе
  спал бы на бэкоффе).

### К §9 (интерфейс)

- Под статусом — предупреждения (`snapshot.warnings` + `job.warnings`
  завершённого задания, до пяти строк) и строка «⚠ Память не обновилась: …
  Следующая попытка — после ЧЧ:ММ или кнопкой «Догнать».»
- У сводки до 2.4.0 вместо «Пересборка прервана» — «Старая сводка переводится
  в новую схему: готово до #N» без «Продолжить».
- «Пакет N» — номер текущего пакета при `merge`/`retry`/`compact`, «После пакета
  N» при `wait`.
- «Догнать» больше трёх пакетов — с подтверждением стоимости; подтверждение
  «Пересобрать» — «плюс факты — только там, где их ещё нет».
- Итог задания: токены — `job.snapshot_tokens`; `processed: 0` — текст
  предупреждения вместо «0 сообщений → 0 токенов»; текст по режиму («Память
  пересобрана» / «Память догнала чат»); отмена — «…остановлено, готовое
  сохранено».
- Строки «из бюджета хода …» и «из лимита модели …» — только во вкладке; в
  инспекторе — полоса и подписи уровней (доля от бюджета — в его шапке). В
  группе вместо полосы — «монитор уровней не считается».
- Снимок в списке записей — свёрнут в `<details>`, с переводами строк.
- Доступность: итог задания — в `aria-live`-регион, фокус после старта, сброса и
  конца задания — на «Остановить» или заголовок блока; метки статуса не
  слипаются на 375 px.
