...
# Подсказка по оформлению: чтобы ответ одинаково аккуратно выглядел и в веб-приложении,
# и в Telegram (там Markdown конвертируется в HTML). Лёгкая разметка, без таблиц/HTML.
STYLE_GUIDE = (
    "[Оформление ответа] Пиши естественной прозой. Лёгкую разметку используй так, "
    "чтобы она одинаково хорошо смотрелась и в приложении, и в Telegram: *курсив* "
    "для действий и мыслей, **жирный** для акцентов, `моноширинный` и блоки кода в "
    "тройных кавычках для технического текста, «> » для цитат, «- » для списков. "
    "Не используй таблицы и HTML-разметку — в Telegram они не отображаются."
)
