...
Теперь контекст собирается трёхслойным конвейером:
  ...
  2. Скользящая сводка (Rolling Hierarchical Compressor) — фоном, каждые
     summary_every сообщений, быстрая модель вливает вышедшие из окна
     сообщения в мастер-снимок: хроника, статусы персонажей, реестр и списки
     (memory_service.run_incremental поверх ядра hierarchical_memory; после хода
     его зовёт main._maybe_update_summary). В контекст он идёт блоком
...
# Окно переехало в ядро иерархической памяти (§4.9 спеки) без изменений
# поведения. Здесь — реэкспорт, а не копия: тесты подменяют именно
# backend.horae_recall.window_start, и horae_memory зовёт его через этот модуль.
# Две копии функции разошлись бы при первой же правке одной из них.
from backend import hierarchical_memory as hm
from backend.hierarchical_memory import WINDOW_STEP, window_start  # noqa: F401 — реэкспорт

logger = logging.getLogger("aichat.horae")

# ---- Слой 1: активное окно -------------------------------------------------

# Сколько последних сообщений идёт в модель дословно. ТЗ: 50–150; 50 — нижняя
# граница: всё, что старше окна, модель видит только через мастер-снимок, и
# окно уже не должно быть тоньше, чем связная сцена. Значение берём из ядра;
# клиент показывает тот же дефолт — держите значения равными.
DEFAULT_WINDOW = hm.DEFAULT_WINDOW
...
# Старую сводку пересобирает новый сводчик (см. memory_service._read_target и
# DbBatchSource), и только его указатель — с этой версией — окно считает правдой.
SUMMARY_FORMAT = 2
...
# ============================================================================
# Слой 1: активное окно (window_start — в hierarchical_memory, см. импорт выше)
# ============================================================================
