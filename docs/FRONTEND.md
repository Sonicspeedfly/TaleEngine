...
- **`horae.js`** — вкладка «Хроника» (Horae State Engine): `window.HoraeUI =
  ...
  состояние. Сеть и тосты — через `$root.api`/`showToast`.
...
- **Порядок списка чатов:** единственный сортировщик — функция `sortChats` вверху
  `app.js`; через неё идут сайдбар (`unifiedChats`: личные чаты, группы,
  расшаренные), палитра (при равной релевантности поиска) и «Недавние диалоги»
  (`recentChats` = первые 5 строк `unifiedChats`). Правило то же, что у сервера
  (`_sort_by_activity` в `main.py`): закреплённые (`pinned`) всегда выше → последняя
  активность (`activity_at` с сервера: последняя реплика либо создание чата, что
  позже), новые выше — и среди закреплённых тоже → ничья по секундам: `sortKey`
  (= `last_id`) → `id`. Сортируется копия, а не реактивный массив. Новый пустой
  чат наверху благодаря времени создания; расшаренные сортируются по своей
  активности и всегда считаются незакреплёнными. Активность никогда не
  перепрыгивает закреп.
- **Живой список чатов (без F5):** любая мутация зовёт
  `notifyChatListChanged(sid)` — событие `taleengine:chatlist-updated` на `window`
  (константа `CHATLIST_EVENT`, дебаунс 120 мс); слушатель в `mounted` перечитывает
  список (`refreshChatList`). Где свежие строки нужны сразу (создание, форк,
  удаление, переименование, закреп, группы, импорт), — `syncChatList(sid)`.
  Сигнал шлют также событие `job` (реплика сохранена), `done`/ошибка стрима,
  свайп/правка/удаление сообщения, арт и канвас, персонажи. Соседние вкладки
  получают тот же сигнал через `BroadcastChannel("taleengine")`
  (`CHATLIST_CHANNEL`; своя вкладка отсеивается по `TAB_ID`, без переотправки);
  если там изменили открытый здесь чат, сообщения перечитываются, когда лента
  простаивает (`_feedIdle`). Реплики из Telegram сервер не присылает — их ловят
  возврат во вкладку (не чаще раза в 10 с) и `_pollChatList` раз в 60 с.
  Открытый чат, пропавший из списка, закрывается только после ответа сервера
  403/404 (`_checkOpenChatAlive`). Список не очищается на время загрузки, ошибка
  оставляет старые строки, фокус в сайдбаре восстанавливается.
...
- **Мобильный композер:** на ≤760px `.composer .row` переносит строку, `.composer-input`
  ...
  ниже, «Отправить» прижат вправо. Композер прижат к низу (`position: sticky`,
  слой `--z-nav`), все плашки над полем (режимы, ошибка, ответ, вложения,
  загрузки) — в `.composer-bars` с потолком 38dvh (30 на телефоне, 22 на низком
  экране) и своей прокруткой; полоса вложений `.att-strip` не выше 110px, на
  телефоне — карусель до 75px. Так никакие вложения не выталкивают «Отправить».
- **Строка действий под репликой:** сверху метаданные (свайпы, время, модель)
  одной строкой, снизу кнопки. В режиме `compactActions` (тач или окно <768px)
  кнопки — горизонтальная полоса с прокруткой: 📋 ✎ ↻ 🗑 и «⋯» первыми, остальное
  (ответ, к началу, продолжить, 🎨, ⑂) — за «⋯» (`msgMenu`, `toggleMsgMenu`).
