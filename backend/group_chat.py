...
async def build_group_messages(
    ...
    # Только техническая часть очерёдности: чей сейчас ход и что нельзя говорить за
    # других. Указания «в характере» здесь намеренно НЕТ — как себя вести, задаёт
    # карточка персонажа, а не приложение (см. BEHAVIOR_GUIDE).
    group_instr = (
        "[Групповой чат] Участники: " + ", ".join(member_names) + ". Ты — "
        + target_character.name + ". Отвечай ТОЛЬКО как " + target_character.name
        + ", одной репликой. Не пиши реплики за других персонажей ("
        + ", ".join(others) + ")."
    )
