...
- 🕰 **Хроника Horae** — перенос плагина Horae из SillyTavern: время сюжета с
  относительными датами, место и память сцен, присутствующие и костюмы, предметы,
  NPC, расположение, планы, отношения, настроение, хронология событий со
  свёртками, вспоминание старых событий, свои таблицы и RPG; настройки — глобально,
  на персонажа и на чат ([docs/HORAE_STATE.md](docs/HORAE_STATE.md)).
- 🖼 **Арты** и мультимодальность: картинки, аудио (запись с микрофона), **видео**, **документы Word/PDF**; предпросмотр вложений (миниатюры, аудио/видеоплеер, лайтбокс).
- 🧮 **LaTeX-формулы** в ответах (KaTeX): `$...$`, `$$...$$`, `\(...\)`, `\[...\]`.
- 🕒 **Время**: метки времени сообщений; нейросеть видит текущее время пользователя (часовой пояс — настройка каждого чата).
- 🛟 **Запасная модель**: при сбое основной ход автоматически (или кнопкой) повторяется резервной моделью.
...
| [docs/HORAE_STATE.md](docs/HORAE_STATE.md) | Хроника Horae: состояние сюжета, теги, свёртки, вспоминание, таблицы, RPG |
...
│  ├─ horae_state.py              # Хроника Horae: теги ответа → мета → состояние → блок промпта
│  ├─ horae_engine.py / _tasks.py # Хроника: сервис БД / ИИ-задачи (анализ, скан, свёртки)
│  ├─ horae_vector.py / _api.py   # Хроника: вспоминание событий / эндпоинты
│  ├─ horae_time/_rpg/_tables/_prompts/_settings/_import.py  # время, RPG, таблицы, промпты, настройки, импорт
│  ├─ horae_prompts_ru/           # русские промпты Horae по умолчанию
...
│  ├─ horae.js                    # вкладка «Хроника» и строка Horae под ответом
