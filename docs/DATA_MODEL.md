...
`attachments` (JSON; `type` = `image|audio|video|document`), `swipes` (JSON — варианты
ответа) + `active_swipe`, `model_used` (какая модель ответила; при срабатывании
запасной — она), `speaker_name` (кто сказал в группе), `reply_to_id` (ответ на
конкретное сообщение), `created_at` (UTC; в API отдаётся ISO-строкой с «Z» —
у user-сообщения это время отправки, у assistant — время готовности ответа).
