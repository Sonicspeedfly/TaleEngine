...
class Settings(BaseSettings):
    ...
    # видит модель на каждый ход. Раньше было 8000 — и это ощущалось «дырой в
    # памяти»: всё старше пары десятков реплик выпадало. Gemini тянет до ~1 млн;
    # 64к — баланс между памятью и ценой/скоростью. Переопределяется в UI
    # («Окно контекста» в Генерации, поле context_tokens).
    CONTEXT_TOKEN_BUDGET: int = 64000
