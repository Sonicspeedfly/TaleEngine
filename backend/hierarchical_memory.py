...
def _parse_entries(lines: list) -> list[list[str]]:
    """
    Записи: строка верхнего уровня (с маркером или без) плюс продолжение —
    строки с отступом (вложенные пункты тоже). Пустые строки не входят в запись
    и не рвут её: вложенный пункт после пустой строки остаётся продолжением.
    """
