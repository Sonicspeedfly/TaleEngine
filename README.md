...
- 🧠 **Память** — всё в одной вкладке: лорбук в духе World Info, активное окно,
  мастер-снимок старой части чата и факты ([docs/HORAE.md](docs/HORAE.md)).
- 🕰 **Хроника Horae** — перенос плагина Horae из SillyTavern: время сюжета с
  ...
  на персонажа и на чат ([docs/HORAE_STATE.md](docs/HORAE_STATE.md)). Старую
  историю сжимает что-то одно — мастер-снимок или свёртки Хроники, — и блоки
  памяти в промпте не повторяют друг друга.
...
**Обновление:** `git pull` и снова `start.sh` / `start.bat`. Перед стартом сервера
скрипт сам доставит изменившиеся зависимости, а если в новой версии изменилась схема
базы — сделает копию в `data/backups/` (последние 5) и обновит базу.
...
│  ├─ horae.js                    # вкладка «Память» (Хроника) и строка Horae под ответом
