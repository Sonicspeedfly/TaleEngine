...
- **Подготовка перед стартом** — `scripts/prestart.py`, его зовут оба скрипта (уже из
  `.venv`), поэтому после `git pull` руками ничего делать не нужно:
  - **зависимости:** метка `.venv/.installed` хранит хэш `backend/requirements.txt`;
    не совпал — `pip install -r`. Упала первая установка — запуск останавливается;
    упала доустановка — запуск со старыми библиотеками и повтор в следующий раз;
  - **база:** `data/.schema-fingerprint` — хэш `backend/models.py` + `backend/database.py`.
    Не совпал — копия SQLite-файла в `data/backups/` (sqlite backup API, хранятся
    последние 5), затем `init_db()` до старта сервера и бота. Не удалась копия или
    миграция — запуск останавливается, отпечаток не обновляется. `SKIP_DB_BACKUP=1` —
    обновить без копии (например, если не хватает места на диске).
